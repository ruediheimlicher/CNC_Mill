 ...
 //  [OpenPanel setAllowedFileTypes:[NSArray arrayWithObjects:@"txt",NULL]];
   ...
   NSURL* ProfilPfad=[OpenPanel URL];
	...
	for (i=0;i<[tempArray count];i++)
	{
		...
		float wertx=[[tempZeilenArray objectAtIndex:0]doubleValue];//*100;
		float werty=[[tempZeilenArray objectAtIndex:1]doubleValue];//*100;
...
- (NSArray*)flipProfil:(NSArray*)profilArray
{
   ...
   for (i=0;i< [profilArray count];i++)
   {
      ...
      float tempx=[[tempZeilenDic objectForKey:@"x"]doubleValue];
      tempx *= -1;
      tempx += 1;
...
- (NSDictionary*)ProfilDatenAnPfad:(NSString*)profilpfad
{
   ...
   int Nasenindex=0;
	...
	for (i=0;i<[tempArray count];i++)
	{
		...
		NSArray* tempZeilenArray=[tempZeilenString componentsSeparatedByString:@"\t"];
		float wertx=[[tempZeilenArray objectAtIndex:0]doubleValue];
		float werty=[[tempZeilenArray objectAtIndex:1]doubleValue];
		NSString*tempX=[NSString stringWithFormat:@"%@", [numberFormatter stringFromNumber:[NSNumber numberWithFloat:wertx]]];
		NSString*tempY=[NSString stringWithFormat:@"%@", [numberFormatter stringFromNumber:[NSNumber numberWithFloat:werty]]];
		//NSLog(@"tempX: %@",tempX);
		//NSDictionary* tempDic = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:wertx], @"x",
...
-  (NSDictionary*)readProfilMitName
{
   ...
   return NULL;
   if (antwort == NSModalResponseCancel)
   {
...
- (NSDictionary*)SplinekoeffizientenVonArray:(NSArray*)dataArray
{
  // NSLog(@"SplinekoeffizientenVonArray l: %d dataArray: %@",[dataArray count],[dataArray description]);
   ...
   for(i=0;i<l;i++)
   {
      x[i] = [[[dataArray objectAtIndex:i] objectForKey:@"x"]doubleValue];
      y[i] = [[[dataArray objectAtIndex:i] objectForKey:@"y"]doubleValue];
   ...
   for(i=0;i<(l-1);i++)
   {
      ...
      //float temp=[[[dataArray objectAtIndex:i+1] objectForKey:@"x"]doubleValue] - [[[dataArray objectAtIndex:i] objectForKey:@"x"]doubleValue];
      
      h[i]= [[[dataArray objectAtIndex:i+1] objectForKey:@"x"]doubleValue] - [[[dataArray objectAtIndex:i] objectForKey:@"x"]doubleValue];
//      NSLog(@"i: %d  h[i]:%f temp: %f",i,h[i],temp);
  ...
  // NSLog(@"\n\n");
   ...
   for(i=0;i<(l-1);i++)
   {
      ...
      e[i]= 6/h[i] * ([[[dataArray objectAtIndex:i+1]objectForKey:@"y"]doubleValue] - [[[dataArray objectAtIndex:i]objectForKey:@"y"]doubleValue]);
...
- (NSArray*)wrenchProfil:(NSArray*)profilArray mitWrench:(float)wrench
{
   ...
   for(i=0;i<[profilArray count];i++)
   {
      ...
      float x1=[[[profilArray objectAtIndex:i]objectForKey:@"x"]doubleValue];
      float y1=[[[profilArray objectAtIndex:i]objectForKey:@"y"]doubleValue];
...
//   NSLog(@"linienArray: %@",[linienArray valueForKey:@"teil"]);
   ...
   float x0=[[[linienArray objectAtIndex:0]objectForKey:@"bx"]doubleValue];
   float y0=[[[linienArray objectAtIndex:[linienArray count]-1]objectForKey:@"by"]doubleValue];
   ...
   x0=[[[linienArray objectAtIndex:profilendeindex]objectForKey:@"bx"]doubleValue];
   y0=[[[linienArray objectAtIndex:profilendeindex]objectForKey:@"by"]doubleValue];
   }
   ...
   for(i=0;i<[linienArray count];i++)
   {
      ...
      float x1=[[[linienArray objectAtIndex:i]objectForKey:@"bx"]doubleValue];
      float y1=[[[linienArray objectAtIndex:i]objectForKey:@"by"]doubleValue];
  ...
  // NSLog(@"Utils openProfil anz: %d temp_r_Array: \n%@",[temp_r_Array count],temp_r_Array);
   ...
   NSString* firstString = [tempArray objectAtIndex:0];
	...
	for (i=0;i<[tempArray count];i++)
	{
      ...
      {
      // object 0 ist index
      float wertx=[[tempZeilenArray objectAtIndex:1]doubleValue];//*100;
		float werty=[[tempZeilenArray objectAtIndex:2]doubleValue];//*100;
		NSString*tempX=[NSString stringWithFormat:@"%@", [numberFormatter stringFromNumber:[NSNumber numberWithFloat:wertx]]];
		NSString*tempY=[NSString stringWithFormat:@"%@", [numberFormatter stringFromNumber:[NSNumber numberWithFloat:werty]]];
		//NSLog(@"tempX: %@",tempX);
		//NSDictionary* tempDic = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:wertx], @"x",
